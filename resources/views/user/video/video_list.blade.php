					...
					<table id='listVideos' class='table listVideos' width='100%'>
						...
						<tbody>
							@foreach($videos as $video)
								...
								<tr>
									<td>{{ $video->title }}</td>
									<td><a href="{{ url('user/videos/pdfdownload') }}?link={{ urlencode($video->file_link) }}&title={{ urlencode($video->title) }}&type={{ getUrlExtension($video->file_link) }}" download="{{ $video->title }}">Download</a></td>
								</tr>
								@endif
							@endforeach
						</tbody>
